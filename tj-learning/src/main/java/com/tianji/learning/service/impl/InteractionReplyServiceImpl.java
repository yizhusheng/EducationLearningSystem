...
public class InteractionReplyServiceImpl extends ServiceImpl<InteractionReplyMapper, InteractionReply> implements IInteractionReplyService {
    ...
    public void saveReply(ReplyDTO replyDTO) {
        ...
        save(reply);
        //3.判断是否是回答,为空则是回答，否则是评论
        LambdaUpdateWrapper<InteractionQuestion> question = new LambdaUpdateWrapper<>();
        //判断是否是回答
        boolean isAnswer = reply.getAnswerId() == null;
        if(isAnswer){
            //3.1.若是回答,修改问题表最近一次回答id
            question.eq(InteractionQuestion::getId, replyDTO.getQuestionId())
            ...
            questionMapper.update(null, question);
    ...
    public PageDTO<ReplyVO> queryReplyPage(ReplyPageQuery query, boolean forAdmin) {
        ...
        //标记当前是在查询问题下的回答
        boolean isQueryQuestion = questionId != null;
        //2.分页查询reply
        Page<InteractionReply> page = lambdaQuery()
                .eq(isQueryQuestion, InteractionReply::getQuestionId, questionId)
                .eq(InteractionReply::getAnswerId, isQueryQuestion ? 0L : answerId)
                .page(query.toMpPage(
